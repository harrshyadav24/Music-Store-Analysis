...
JOIN genre ON genre.genre_id = track.genre_id
WHERE genre.name LIKE 'Rock'
GROUP BY artist.artist_id, artist.name  -- Add artist.name to GROUP BY
...
GROUP BY c.customer_id, c.first_name, c.last_name, bsa.artist_name  -- Add customer and artist details to GROUP BY
...
(
	...
	JOIN track ON track.track_id = invoiceline.track_id
	JOIN genre ON genre.genre_id = track.genre_id
	GROUP BY customer.country, genre.name, genre.genre_id  -- Add genre.name and genre_id to GROUP BY
...
WITH Customer_with_country AS (
	...
	FROM invoice
	JOIN customer ON customer.customer_id = invoice.customer_id
	GROUP BY customer.customer_id, first_name, last_name, billing_country  -- Add first_name, last_name, billing_country to GROUP BY
